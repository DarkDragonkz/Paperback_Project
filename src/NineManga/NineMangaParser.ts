...
export class NineMangaParser {
  ...
  parseSourceSelectionUrl(html: string): string | undefined {
    ...
    const allowed =
      ...
      /type=(?:en|es|ru)ninemanga/i.test(normalizedUrl)
  ...
  private extractUrlLikeValues(value: string): string[] {
    ...
    if (
      ...
      /(?:en|es|ru)ninemanga/i.test(trimmed)
    ) {
      values.push(trimmed)
    }
  ...
  private isUsefulGateCandidateUrl(url: string): boolean {
    ...
    const isNineMangaChapter =
      /^https?:\/\/(?:www\.)?(?:[a-z]{2}\.)?ninemanga\.com\//i.test(url) &&
      normalized.includes('/chapter/')
    const isKnownGate =
      normalized.includes('/go/ennm/') ||
      normalized.includes('/go/jump/') ||
      /type=(?:en|es|ru)ninemanga/i.test(normalized) ||
  ...
  private isNineMangaUrl(url: string): boolean {
    try {
      ...
      const baseHost = new URL(this.baseUrl).host.toLowerCase().replace(/^www\./, '')

      return host === baseHost || host.endsWith('.ninemanga.com')
    } catch {
