...
export class NineMangaClient {
  ...
  private readonly localizedReaderCache = new Map<string, CacheEntry<string[]>>()
  private readonly localizedReaderRequests = new Map<string, Promise<string[]>>()
  ...
  private clearLanguageSensitiveCaches(): void {
    ...
    this.localizedReaderCache.clear()
    this.localizedReaderRequests.clear()
  }
...
private chapterProgressionNumber(chapter: Chapter): number {
  ...
  private async resolveReaderImages(chapter: Chapter, chapterUrl: string): Promise<string[]> {
    ...
    const state: ReaderResolutionState = {
      ...
      chapterId,
      financePostId: cachedFinancePostId,
  ...
  private async resolveReaderResponse(
    ...
    if (this.config.flowType === 'localized-tascabile') {
      return this.resolveLocalizedTascabileReaderResponse(response, state)
    }
  ...
  private async resolveLocalizedTascabileReaderCached(
    chapterUrl: string,
    state: ReaderResolutionState
  ): Promise<string[]> {
    const cacheKey = this.sourceFlowKey(this.withReaderWarning(normalizeUrl(chapterUrl, this.baseUrl())))
    ...
    const request = this.resolveLocalizedTascabileReader(chapterUrl, state)
      .then((pages) => {
        const uniquePages = uniqueStrings(pages)
        if (uniquePages.length > 0) {
          this.rememberCache(this.localizedReaderCache, cacheKey, uniquePages, HTML_CACHE_TTL_MS)
        }
        return uniquePages
      })
      .finally(() => this.localizedReaderRequests.delete(cacheKey))

    this.localizedReaderRequests.set(cacheKey, request)
    return request
  }

  private async resolveLocalizedTascabileReader(
    chapterUrl: string,
    state: ReaderResolutionState
  ): Promise<string[]> {
  ...
  private async resolveLocalizedTascabileReaderResponse(
    firstResponse: TextResponse,
    state: ReaderResolutionState
  ): Promise<string[]> {
    const pages: string[] = []
    const firstUrl = this.withReaderWarning(normalizeUrl(firstResponse.url, this.baseUrl()))
  ...
  private localizedChapterBaseKey(url: string): string {
  ...
  private financeMasterProAlternateUrls(
    ...
    const headerPostId = this.financeWordPressPostIdFromHeaders(headers)
    const htmlPostId = this.financeWordPressPostIdFromHtml(html)
    const fallbackReaderPageId = this.financeFallbackReaderPageIdForChapter(state.chapterId)
  ...
  private financeWordPressPostIdFromHtml(html: string): string {
    const candidate = (
  ...
  private financeWordPressPostIdFromHeaders(headers: Record<string, string>): string {
...
private isFinanceChapterEndpointUrl(url: string): boolean {
  ...
  private rememberFinanceReaderPageId(
    url: string,
    state: ReaderResolutionState,
    cache = true
  ): void {
    const readerInfo = this.financeReaderInfoFromUrl(url, state.chapterId ?? '')
  ...
  private isNineMangaUrl(url: string): boolean {
    try {
      ...
      return host === cfgHost || host.endsWith('.ninemanga.com')
    } catch {
      return /^https?:\/\/(?:www\.)?(?:[a-z]{2}\.)?ninemanga\.com(?:[/:?#]|$)/i.test(url)
    }
  }
...
private financeJumpUrlForState(state: ReaderResolutionState): string {
  ...
  private applyFinanceGateCookies(state: ReaderResolutionState): void {
    const financePostId = state.financePostId
