...
export async function head(url: string, headers?: HeaderMap): Promise<HeaderResponse> {
  return requestHeaders({ url, method: 'HEAD', headers }, 0)
}
...
async function requestHeaders(request: Request, redirectCount: number): Promise<HeaderResponse> {
