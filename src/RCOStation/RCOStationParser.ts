...
export class RCOStationParser {
  ...
  private parseInlineReaderImageUrls(html: string): string[] {
    ...
    const directImages = uniqueStrings(images)
    if (directImages.length > 0) return directImages

    return this.parseProtectedReaderImageUrls(decodedHtml)
  }
  ...
  private isValidReaderImage(url: string): boolean {
    ...
    if (this.isMalformedBlogspotUrl(url)) return false
  ...
  private isMalformedBlogspotUrl(url: string): boolean {
    ...
    if (/%(?:00|03|04|05|06|07|08|09|0a|0d|10|12|13|14|15|16|17|1f|7f|c2|c3)/i.test(path)) return true
    if (!/=(?:s0|s1600)(?:[?#]|$)/i.test(url)) return true
